...
void loop() {
  ...
  if (scanResults == WIFI_SCAN_RUNNING) {
    ...
    if (Firebase.ready()) {
      if (Firebase.RTDB.setInt(&fbdo, "B_wifi_device_count_this_loop", wifiDeviceCountThisLoop)) {
      ...
      if (Firebase.RTDB.setInt(&fbdo, "B_total_wifi_device_count", wifiTotalDeviceCount)) {
  ...
  if (Firebase.ready()) {
    if (Firebase.RTDB.setInt(&fbdo, "B_ble_device_count_this_loop", bleDeviceCountThisLoop)) {
    ...
    if (Firebase.RTDB.setInt(&fbdo, "B_total_ble_device_count", bleTotalDeviceCount)) {
  ...
  delay(60000); // Adjust delay 1 Minute
}
